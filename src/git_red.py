...
def get_git_log(since_date: str) -> list[str]:
    ...
    try:
        ...
        return (
            [line.split("\t", 1) for line in log_output.split("\n") if line]
            if log_output
            else []
        )
...
def get_commit_diff_stats(commit_hash: str) -> tuple[int, int]:
    ...
    try:
        diff_stat = subprocess.check_output(
            ["git", "show", "--stat", commit_hash], text=True
        )
        # The last line which contains the summary
        # Example: 4 files changed, 172 insertions(+), 122 deletions(-)
        last_line = diff_stat.strip().split("\n")[-1]
        added = 0
        deleted = 0
        if "insertions" in last_line:
            added = int(last_line.split("insertions")[0].split()[-1])
        if "deletions" in last_line:
            deleted = int(last_line.split("deletions")[0].split()[-1])
        ...
        if total_lines >= min_lines:
            ...
            if percent_deleted >= min_pct:
                commit_author = subprocess.check_output(
                    ["git", "log", "-1", "--pretty=format:%an", commit_hash], text=True
                ).strip()
                add_str = f"+{added}"
                del_str = f"-{deleted}"
                print(
